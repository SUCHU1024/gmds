	...
	$(".type").dblclick(function(){
		...
		$.ajax({
			...
			success:function(r){
				...
				//	console.log("j.id="+list[i].gtid+ "name="+list[i].name+"description="+list[i].description);
					var $option = $("<option>").text(list[i].name).val(list[i].gtid);	//下拉框构造
...
//	alert("要修改的商品id=" + goodsid+ "类型id="+ gtid);
	//1、写一个web服务方法
	//2、在这里写ajax请求，调用上面的方法
	//3、在业务层接口扩展一个更新一个更行商品类别的方法
	//4、在业务层实现类实现相应的方法
	//5、在数据层DAO接口扩展一个更新商品
	$.ajax({
...
function baocun(a){
	...
	var $input2 = $tr.children("td:eq(3)").children("input"); //获取第二个文本框
	var $input3 = $tr.children("td:eq(4)").children("input"); //获取第三个文本框
	var $input4 = $tr.children("td:eq(5)").children("input"); //获取第四个文本框
	var goodsid = $tr.children("td:eq(0)").children("input").val();
	var data = {
			"goodsid":goodsid,
			"name":$input1.val(),"spec":$input2.val(),
